...
kb = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="ℹ️ Über uns")],
...
sell_kb = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="🔙 Zurück zu Dienstleistungen")],
    ],
    resize_keyboard=True,
)
...
rent_kb = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="Kontakt für Vermietung")],
        [KeyboardButton(text="🔙 Zurück zu Dienstleistungen")],
    ],
    resize_keyboard=True,
)
...
sale_kb = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="💬 Kostenlose Einschätzung anfordern")],
        [KeyboardButton(text="🔙 Zurück")],
    ],
    resize_keyboard=True,
)
...
async def cmd_start(m: Message):
    ...
    if os.path.isfile(photo_path):
        ...
        await m.answer_photo(
            ...
            caption=(
                "Willkommen!\n"
                "Ich bin Ihr virtueller Assistent für Immobilienfragen in der Schweiz.\n"
...
async def about_me(m: Message):
    ...
    if os.path.isfile(photo_path):
        ...
        caption = (
            ...
            "Ihre Immobilie ist in zuverlässigen Händen – von der Bewertung bis zur Schlüsselübergabe.\n\n"
            "Möchten Sie direkt Kontakt aufnehmen?"
        )
        await m.answer_photo(
            photo=photo,
            caption=caption,
...
async def sale_process(m: Message):
    text = (
        "So läuft ein professioneller Immobilienverkauf ab:\n"
...
async def price_list(m: Message):
    ...
    await m.answer(
        "Bitte wählen Sie den Bereich, der Sie interessiert:",
...
async def prices_valuation(m: Message):
    await m.answer("Bitte wählen Sie den Objekttyp:", reply_markup=valuation_type_kb)
...
@dp.message(F.text == "💬 Kostenlose Einschätzung anfordern")
async def sale_request(m: Message):
    await m.answer("Vielen Dank — wir werden uns für die Einschätzung melden.", reply_markup=kb)
    await contacts(m)
...
async def free_first_consult(m: Message):
    ...
    await m.answer("Möchten Sie eine kostenlose Erstberatung? Hier unsere Kontaktinformationen:", reply_markup=kb)
    await contacts(m)
...
@dp.message(F.text == "🔙 Zurück zum Hauptmenü")
async def back_to_main(m: Message):
    await m.answer("Zurück zum Hauptmenü.", reply_markup=kb)


@dp.message(F.text == "🔙 Zurück zu Dienstleistungen")
async def back_to_services(m: Message):
    await services_menu(m)


@dp.message(F.text == "Kontakt für Vermietung")
...
async def valuation_apartment(m: Message):
    await m.answer("Sie haben 'Wohnung' ausgewählt. Bitte kontaktieren Sie uns für ein genaues Angebot.", reply_markup=kb)


@dp.message(F.text == "Einfamilienhaus")
async def valuation_house(m: Message):
    await m.answer("Sie haben 'Einfamilienhaus' ausgewählt. Bitte kontaktieren Sie uns für ein genaues Angebot.", reply_markup=kb)


@dp.message(F.text == "Gewerbeobjekt")
async def valuation_commercial(m: Message):
    await m.answer("Sie haben 'Gewerbeobjekt' ausgewählt. Bitte kontaktieren Sie uns für ein genaues Angebot.", reply_markup=kb)
